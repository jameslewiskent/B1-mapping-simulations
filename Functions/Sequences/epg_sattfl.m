...
if settings.UseSyntheticData == 1
    ...
    IT1 = zeros(settings.Scan_Size(1),settings.Scan_Size(2),size(settings.Tx_FA_map,3),length(settings.Dynamic_Range),length(settings.B0_Range_Hz),length(settings.T1s),length(settings.Velocities),length(settings.Diff_coeffs));
    IT2 = zeros(settings.Scan_Size(1),settings.Scan_Size(2),size(settings.Tx_FA_map,3),length(settings.Dynamic_Range),length(settings.B0_Range_Hz),length(settings.T1s),length(settings.Velocities),length(settings.Diff_coeffs));
    Total_Energy = zeros(1,length(settings.Dynamic_Range));
            ...
            for B0_n = 1:length(settings.B0_Range_Hz)
                for Flow_n = 1:length(settings.Velocities)
                    ...
                    for Diff_n = 1:length(settings.Diff_coeffs)
                        ...
                        [IT1(:,:,:,Dynamic_Range_n,B0_n,T1_n,Flow_n,Diff_n),IT2(:,:,:,Dynamic_Range_n,B0_n,T1_n,Flow_n,Diff_n),Cumulative_Time,N_imaging_RF,N_prep_RF,Mag_Track] = SatTFL_Seq(T1,IT_FAs,PP_FAs,Velocity,Angle,Diff_co,settings);
...
% Sequence Function
    function [Train1,Train2,Cumulative_Time,N_imaging_RF,N_prep_RF,Mag_Track] = SatTFL_Seq(T1,IT_FAs,PP_FAs,Velocity,Angle,Diff_co,settings)
