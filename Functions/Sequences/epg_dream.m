...
%       mapping scheme using the images formed from a STEAM preperation
    ...
    for Dynamic_Range_n = 1:length(settings.Dynamic_Range)
        for T1_n = 1:length(settings.T1s)
            for B0_n = 1:length(settings.B0_Range_Hz)
                ...
                IT_FAs = Simulate_RF_Pulse(settings.Hz_per_Volt*settings.Dynamic_Range(Dynamic_Range_n)*settings.IT_RF_Pulse,settings.IT_RF_Time,settings.B0_Range_Hz(B0_n)+settings.Slice_Shifts,T1,settings.T2); % Output FA in radians
                PP_FAs = Simulate_RF_Pulse(settings.Hz_per_Volt*settings.Dynamic_Range(Dynamic_Range_n)*settings.PP_RF_Pulse,settings.PP_RF_Time,settings.B0_Range_Hz(B0_n)+settings.PP_Shifts,T1,settings.T2); % Output FA in radians
                settings = Check_Mag_Track_Flag(Dynamic_Range_n,T1_n,B0_n,settings);
                [IT1(:,:,:,Dynamic_Range_n,B0_n,T1_n),IT2(:,:,:,Dynamic_Range_n,B0_n,T1_n),Cumulative_Time,N_imaging_RF,N_prep_RF,Mag_Track] = DREAM_Seq(T1,IT_FAs,PP_FAs,settings);
...
% Sequence Function
    function [Train1,Train2,Cumulative_Time,N_imaging_RF,N_prep_RF,Mag_Track] = DREAM_Seq(T1,IT_FAs,PP_FAs,settings)
        ...
        for Mode_n = 1:size(settings.Tx_FA_map,3) % repeat for different modes of mTx array
            ...
            [P,~,~,Mag_Track] = epg_grelax(P,T1,settings.T2,time1,settings.kg,settings.Diff_co,0,0,settings.Velocity,settings.Angle,Mag_Track,settings); % relaxation
            ...
            N_prep_RF = N_prep_RF + 1;
            [P,~,Mag_Track] = epg_rf(P,PP_FA,settings.RF_Phase(Mode_n),settings.PP_RF_Time,Mag_Track,settings); % 2nd STEAM RF alpha pulse
            
            time2 = settings.Td - settings.Ts;
            Cumulative_Time = Cumulative_Time + time2;
            [P,~,~,Mag_Track] = epg_grelax(P,T1,settings.T2,time2,settings.kg,settings.Diff_co,0,0,settings.Velocity,settings.Angle,Mag_Track,settings); % relaxation
            ...
            for PE2_n = 1:settings.Scan_Size(2)
                ...
                for IT_n = 1:settings.Scan_Size(1)
                    ...
                    Cumulative_Time = Cumulative_Time + settings.TE_STE;
                    [P,~,~,Mag_Track] =  epg_grelax(P,T1,settings.T2,settings.TE_STE,settings.kg,settings.Diff_co,0,0,settings.Velocity,settings.Angle,Mag_Track,settings); % relaxation time = TE_STE
                    ...
                    Cumulative_Time = Cumulative_Time + (settings.TE_FID - settings.TE_STE);
                    [P,~,~,Mag_Track] =  epg_grelax(P,T1,settings.T2,settings.TE_FID - settings.TE_STE,settings.kg,settings.Diff_co,0,0,settings.Velocity,settings.Angle,Mag_Track,settings); % relaxation time = TE_FID - TE_STE
                    ...
                    Cumulative_Time = Cumulative_Time + (settings.TR - settings.TE_FID);
                    [P,~,~,Mag_Track] =  epg_grelax(P,T1,settings.T2,settings.TR - settings.TE_FID,settings.kg,settings.Diff_co,0,0,settings.Velocity,settings.Angle,Mag_Track,settings); % relaxation time = TR - TE_FID
