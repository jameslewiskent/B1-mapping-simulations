function [FpFmZ,EE,BV,Mag_Track] = epg_grelax(FpFmZ,T1,T2,Time,kg,D,Gon,noadd,Velocity,Angle,Mag_Track,settings)
...
N_Samples = ceil(Time./settings.Mag_Track_dt); 
T = Time/N_Samples; 
else
    T = Time;
    N_Samples = 1;
...
for n = 1:N_Samples % JK added for tracking magnetisation. Repeat with smaller time increments for magnetisation plotting
...
if nargin > 1			% Skip relaxation if only one argument
  ...
  RR = 1-E1;			% Mz Recovery, affects only Z0 state, as recovered magnetization is not dephased.

  FpFmZ = EE * FpFmZ;		% Apply Relaxation
  FpFmZ(3,1) = FpFmZ(3,1)+RR;	% Recovery  ( here applied before diffusion, but could be after or split.)
end
