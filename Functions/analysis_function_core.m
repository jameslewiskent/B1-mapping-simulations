...
if strcmpi(settings.Scheme,'DREAM')  % Don't re-order DREAM k-space
    ...
    Reordered_IT2 = zeros(size(IT2));
    
    if settings.T1Corr == 1
        Reordered_IT3 = zeros(size(IT3));
    end
    ...
    for IT_n = 1:settings.PE1_Resolution*settings.PE1_Partial_Fourier*settings.Matrix_Size(1)
        ...
        if settings.T1Corr == 1
            Reordered_IT3(Reorder_PE1(2,IT_n),:,:,:,:,:,:,:,:,:) = IT3(IT_n,:,:,:,:,:,:,:,:,:);
        end
    end
end
clearvars IT1 IT2 IT3 Reorder
...
if settings.UseSyntheticData == 0
    ...
    if settings.T1Corr == 1
        Reordered_IT3 = Reordered_IT3(:,settings.Centre_Slice,:,:,:,:,:,:,:,:);
    end
end
...
IT1_Noisy = Reordered_IT1 + bsxfun(@times,randn([size(Reordered_IT1,1:8),size(settings.Noise,2),settings.Repeats]),reshape(Noise_sd,[1 1 1 1 1 1 1 1 size(settings.Noise,2) 1])) + 1i.*bsxfun(@times,randn([size(Reordered_IT1,1:8),size(settings.Noise,2),settings.Repeats]),reshape(Noise_sd,[1 1 1 1 1 1 1 1 size(settings.Noise,2) 1]));
IT2_Noisy = Reordered_IT2 + bsxfun(@times,randn([size(Reordered_IT2,1:8),size(settings.Noise,2),settings.Repeats]),reshape(Noise_sd,[1 1 1 1 1 1 1 1 size(settings.Noise,2) 1])) + 1i.*bsxfun(@times,randn([size(Reordered_IT2,1:8),size(settings.Noise,2),settings.Repeats]),reshape(Noise_sd,[1 1 1 1 1 1 1 1 size(settings.Noise,2) 1]));
if settings.T1Corr == 1
    IT3_Noisy = Reordered_IT3 + bsxfun(@times,randn([size(Reordered_IT3,1:8),size(settings.Noise,2),settings.Repeats]),reshape(Noise_sd,[1 1 1 1 1 1 1 1 size(settings.Noise,2) 1])) + 1i.*bsxfun(@times,randn([size(Reordered_IT3,1:8),size(settings.Noise,2),settings.Repeats]),reshape(Noise_sd,[1 1 1 1 1 1 1 1 size(settings.Noise,2) 1]));
end
...
if settings.PE1_Partial_Fourier ~= 1 || settings.PE1_Resolution ~= 1
    ...
    if settings.T1Corr == 1
        IT3_Noisy = cat(1,cat(1,PadTop, IT3_Noisy),PadBot);
    end
end
...
if (settings.PE2_Partial_Fourier ~= 1 || settings.PE2_Resolution ~= 1) && settings.Matrix_Size(2) > 1
    ...
    if settings.T1Corr == 1
        IT3_Noisy = cat(2,cat(1,PadTop, IT3_Noisy),PadBot);
    end
end
clearvars PadTop PadBot
...
for f = fftdims
    ...
    if settings.T1Corr == 1
        IT3_Noisy =  ifftshift(ifft(fftshift(IT3_Noisy,f),[],f),f);
    end
end
FT_IT1 = IT1_Noisy;
FT_IT2 = IT2_Noisy;
...
clearvars IT1_Noisy IT2_Noisy IT3_Noisy

if settings.UseSyntheticData == 0 && size(FT_IT1,4) ~= 1 && ~isfield(settings,'LoopValues')
    ...
    xlabel('Index');
    if settings.T1Corr == 1
        nexttile; mesh(abs(squeeze(FT_IT3(:,centre_index,1,:,1,1,1,1,1,1,1))))
        caxis([0 7])
        title('Image Train 3')
        ylabel('Pre-pulse FA');
        xlabel('Index');
...
FWHM = zeros([2,2,size(FT_IT1,4:ndims(FT_IT1))]);
if settings.Calc_FWHM == 1 % Measure FWHM (can take a while)
    for Dynamic_Range_n = 1:size(FT_IT1,4)
                    ...
                    for Diff_n = 1:size(FT_IT1,8)
                        for Noise_n = 1:size(FT_IT1,9)
                            for Repeat_n = 1:size(FT_IT1,10)
                                FWHM(1,1,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n) = findFWHM(FT_IT1(:,floor(size(FT_IT1,2)/2 +1),:,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n));
                                FWHM(2,1,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n) = findFWHM(FT_IT2(:,floor(size(FT_IT1,2)/2 +1),:,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n));
                                if settings.T1Corr == 1
                                    FWHM(3,1,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n) = findFWHM(FT_IT3(:,floor(size(FT_IT1,2)/2 +1),:,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n));
                                end
                                
                                if size(FT_IT1,2) > 1
                                    FWHM(1,2,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n) = findFWHM(FT_IT1(floor(size(FT_IT1,1)/2 +1),:,:,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n)');
                                    FWHM(2,2,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n) = findFWHM(FT_IT2(floor(size(FT_IT1,1)/2 +1),:,:,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n)');
                                    if settings.T1Corr == 1
                                        FWHM(3,2,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n) = findFWHM(FT_IT3(floor(size(FT_IT1,1)/2 +1),:,:,Dynamic_Range_n,B0_Range_n,T1_n,Flow_n,Diff_n,Noise_n,Repeat_n)');
                                    end
                                end
                            end
                        end
        ...
        if settings.verbose == 1
            disp(['Calculated FWHM for DR: ',num2str(Dynamic_Range_n),' of ',num2str(size(FT_IT1,4)), ' completed.'])
        end
    end
end

if settings.Sum_PSF == 0 % Take centre of PSF
    Max_Val_IT1 = max(FT_IT1(floor(size(FT_IT1,1)/2 +1),floor(size(FT_IT1,2)/2 +1),:,:,:,:,:,:,:,:),[],1); % Find index of maximum value
    Max_Val_IT2 = max(FT_IT2(floor(size(FT_IT1,1)/2 +1),floor(size(FT_IT1,2)/2 +1),:,:,:,:,:,:,:,:),[],1); % Store maximum value of IT2
    if settings.T1Corr == 1
        Max_Val_IT3 = max(FT_IT3(floor(size(FT_IT1,1)/2 +1),floor(size(FT_IT1,2)/2 +1),:,:,:,:,:,:,:,:),[],1); % Store maximum value of IT3
    end
elseif settings.Sum_PSF == 1 % Sum PSF
    Max_Val_IT1 = sum(FT_IT1,1);
    Max_Val_IT2 = sum(FT_IT2,1);
    if settings.T1Corr == 1
        Max_Val_IT3 = sum(FT_IT3,1);
    end
end
clearvars FT_IT1 FT_IT2 FT_IT3 index
