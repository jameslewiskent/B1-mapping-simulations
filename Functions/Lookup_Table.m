...
% Choose T1, On resonance and average over repeats
fx = squeeze(mean(Image_Train_Ratio(:,:,:,:,settings.B0_Range_Hz == 0,settings.T1s == settings.Lookup_T1,1,:),8)); 
...
for Dynamic_Range_n = 1:size(Image_Train_Ratio,4)
    for B0_n = 1:size(Image_Train_Ratio,5)
        for T1_n = 1:size(Image_Train_Ratio,6)
            for Noise_n = 1:size(Image_Train_Ratio,7)
                for Repeat_n = 1:size(Image_Train_Ratio,8)
                    [~,min_ind] = min(abs(Image_Train_Ratio(1,1,1,Dynamic_Range_n,B0_n,T1_n,Noise_n,Repeat_n) - fx_interp));
                    Measured_FA(1,1,Mode_n,Dynamic_Range_n,B0_n,T1_n,Noise_n,Repeat_n) = x_query(min_ind)*settings.nomPP_FA; % Measured_FA in radians
                end
            end
        end
    end
